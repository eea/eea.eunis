    ...
    <stripes:layout-component name="contents">
        ...
        <ul class="eunistree">
            ...
            <li>
                ...
                <a rel="nofollow" title="<%=hide%>" id="level_<%=h.getIdHabitat()%>" href="habitats-annex1-browser.jsp?expand=<%=Utilities.removeFromExpanded(expand,h.getIdHabitat())%>#level_<%=h.getIdHabitat()%>"><img src="images/img_minus.gif" alt="<%=hide%>"/></a>
                <%=h.getCode()%> : <%=h.getName()%>
                <% } else if (h.hasChildren()) { %>
                <a rel="nofollow" title="<%=show%>" id="level_<%=h.getIdHabitat()%>" href="habitats-annex1-browser.jsp?expand=<%=Utilities.addToExpanded(expand,h.getIdHabitat())%>#level_<%=h.getIdHabitat()%>"><img src="images/img_plus.gif" alt="<%=show%>"/></a>
                <%=h.getCode()%> : <%=h.getName()%>
                <% } else { %>
