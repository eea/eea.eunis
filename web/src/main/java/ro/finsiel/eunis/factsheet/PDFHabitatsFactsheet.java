...
public class PDFHabitatsFactsheet {
    ...
    final Font fontNormal = FontFactory.getFont(FontFactory.HELVETICA, 9,
            Font.NORMAL);
    final Font fontNormalBold = FontFactory.getFont(FontFactory.HELVETICA, 9,
            Font.BOLD);
    final Font fontTitle = FontFactory.getFont(FontFactory.HELVETICA, 12,
            Font.BOLD);
    final Font fontSubtitle = FontFactory.getFont(FontFactory.HELVETICA, 10,
            Font.BOLD);
    ...
    private void getGeneralInformation() throws Exception {
        ...
        cell = new Cell(
                new Phrase(factsheet.getHabitatScientificName(), fontTitle));
        ...
        if (factsheet.isEunis()) {
            ...
            cell = new Cell(
                    new Phrase(
                            Utilities.formatString(factsheet.getCode2000(), " "),
                            fontNormalBold));
            cell.setBackgroundColor(new Color(0xDD, 0xDD, 0xDD));
            table.addCell(cell);
        ...
        if (descriptions.size() > 0) {
            ...
            for (int i = 0; i < descriptions.size(); i++) {
                DescriptionWrapper description = (DescriptionWrapper) descriptions.get(
                        i);
                ...
                if (description.getLanguage().equalsIgnoreCase("english")) {
                    ...
                    cell = new Cell(
                            new Phrase(description.getDescription(), fontNormal));
                    table.addCell(cell);
        ...
        if (names.size() > 0) {
            ...
            for (int i = 0; i < names.size(); i++) {
                Chm62edtHabitatInternationalNamePersist name = (Chm62edtHabitatInternationalNamePersist) names.get(
                        i);
        ...
        if (otherClassifHabitats.size() > 0 || otherHabitats.size() > 0) {
            ...
            for (int j = 0; j < otherClassifHabitats.size(); j++) {
                OtherClassificationPersist otherClassifHabitat = (OtherClassificationPersist) otherClassifHabitats.get(
                        j);

                cell = new Cell(
                        new Phrase(otherClassifHabitat.getName(), fontNormal));
                table.addCell(cell);

                cell = new Cell(
                        new Phrase(otherClassifHabitat.getCode(), fontNormal));
                table.addCell(cell);

                cell = new Cell(
                        new Phrase(otherClassifHabitat.getTitle(), fontNormal));
                table.addCell(cell);

                cell = new Cell(
                        new Phrase(
                                HabitatsFactsheet.mapHabitatsRelations(
                                        otherClassifHabitat.getRelationType()),
                                        fontNormal));
                table.addCell(cell);
            }

            for (int i = 0; i < otherHabitats.size(); i++) {
                HabitatFactsheetRelWrapper otherHab = (HabitatFactsheetRelWrapper) otherHabitats.get(
                        i);
                ...
                cell = new Cell(
                        new Phrase(otherHab.getScientificName(), fontNormal));
                table.addCell(cell);
    ...
    private void getGeographicalDistribution() throws Exception {
        ...
        if (results.size() > 0) {
            ...
            for (int i = 0; i < results.size(); i++) {
                HabitatCountryPersist country = (HabitatCountryPersist) results.get(
                        i);
                ...
                cell = new Cell(
                        new Phrase(country.getBiogeoregionName(), fontNormal));
                table.addCell(cell);

                cell = new Cell(
                        new Phrase(
                                (HabitatsFactsheet.getProbabilityAndCommentForHabitatGeoscope(country.getIdReportAttributes())).get(0).toString(),
                                fontNormal));
                table.addCell(cell);

                cell = new Cell(
                        new Phrase(
                                (HabitatsFactsheet.getProbabilityAndCommentForHabitatGeoscope(country.getIdReportAttributes())).get(1).toString(),
                                fontNormal));
                table.addCell(cell);
            }
            report.addTable(table);
    ...
    private void getRelatedSites() throws Exception {
        ...
        if (sitesForSubtypes.size() > 0) {
            ...
            for (int i = 0; i < sitesForSubtypes.size(); i++) {
                SitesByNatureObjectPersist site = (SitesByNatureObjectPersist) sitesForSubtypes.get(
                        i);
    ...
    private void getRelatedSpecies() throws Exception {
        ...
        if (!species.isEmpty()) {
            ...
            for (int i = 0; i < species.size(); i++) {
                HabitatsSpeciesWrapper wrapper = (HabitatsSpeciesWrapper) species.get(
                        i);

                cell = new Cell(
                        new Phrase(
                                Utilities.formatString(wrapper.getSpeciesName()),
                                fontNormal));
                table.addCell(cell);
